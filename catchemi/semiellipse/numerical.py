...
class NewnsAndersonNumerical:
    ...
    def __post_init__(self):
        ...
        if self.verbose:
            print(
                f"Solving the Newns-Anderson model for eps_a = {self.eps_a:1.2f} eV",
    ...
    def get_dos_on_grid(self) -> np.ndarray:
        ...
        for i, eps_ in enumerate(self.eps):
            ...
            denominator = (eps_function(eps_) - Lambda(eps_)) ** 2 + (
                Delta(eps_) + Delta0(eps_)
            ) ** 2
    ...
    def find_poles_green_function(self) -> list:
        ...
        try:
            pole_lower = optimize.brentq(
                lambda x: eps_function(x) - Lambda(x),
                self.eps_min,
                self.eps_d - self.wd,
            )
    ...
    def _create_dos(self, eps) -> acb:
        ...
        denominator = (eps_function(eps) - Lambda(eps)) ** 2 + (
            Delta(eps) + Delta0(eps)
        ) ** 2
    ...
    def _calculate_filling(self) -> float:
        ...
        filling_numerator = integrate.quad(
            self._create_Delta_reg, self.eps_min, 0, limit=100
        )[0]
        # Filling contribution coming from the sp-states
        filling_numerator += integrate.quad(
            self._create_Delta0_reg, self.eps_min, 0, limit=100
        )[0]
        # Filling contribution to the denomitor coming from the d-states
        filling_denominator = integrate.quad(
            self._create_Delta_reg, self.eps_min, self.eps_max, limit=100
        )[0]
        # Filling contribution to the denomitor coming from the sp-states
        filling_denominator += integrate.quad(
            self._create_Delta0_reg, self.eps_min, self.eps_max, limit=100
        )[0]
    ...
    def calculate_occupancy(self):
        ...
        if self.Delta0_mag == 0:
            ...
            for pole in self.poles:
                if pole is not None:
                    ...
                    if pole.real < 0:
                        ...
                        if (
                            pole.real < self.eps_d.real - self.wd.real
                            ...
                            localised_occupancy += acb("1.0") / (
                                acb("1.0") - Lambda_prime
                            )
            ...
            self.occupancy = acb.integral(
                lambda x, _: self._create_dos(x),
    ...
    def calculate_hybridisation_energy(self):
        ...
        delta_E_ = integrate.quad(
            self._create_energy_integrand,
            self.eps_min,
            0,
        ...
        if (
            ...
            and self.hybridisation_energy < self.NUMERICAL_NOISE_THRESHOLD
        ):
            self.hybridisation_energy = 0
