...
class NewnsAndersonAnalytical:
    ...
    def self_consistent_calculation(self):
        ...
        if self.U == 0:
            ...
            self.n_minus_sigma = 1.0
            self.n_plus_sigma = 1.0
        else:
            ...
            self.energies_grid = np.zeros(
                (len(self.nsigma_range), len(self.nmsigma_range))
            )
            for j, n_down in enumerate(self.nmsigma_range):
                ...
                if lowest_energy is not None:
                    lowest_energy = (
                        maximum_energy
                        if maximum_energy <= lowest_energy
                        else lowest_energy
                    )
                    index_nup_overall = (
                        index_nup
                        if maximum_energy <= lowest_energy
                        else index_nup_overall
                    )
                    index_ndown_overall = (
                        j if maximum_energy <= lowest_energy else index_ndown_overall
                    )
                else:
    ...
    def calculate_energies(self):
        ...
        rho_aa_ += (
            -2 * self.eps_wrt_d * self.eps_sigma_wrt_d * (1 - 2 * self.beta_p**2)
        )
        ...
        if not self.has_complex_root:
            if self.beta_p != 0.5:
                ...
                root_positive += (
                    2
                    * self.beta_p**2
                    * (4 * self.beta_p**2 + self.eps_sigma_wrt_d**2 - 1) ** 0.5
                )
            ...
            root_positive = (
                1 - 2 * self.beta_p**2
            ) * self.eps_sigma + 2j * self.beta_p**2 * (
                1 - 4 * self.beta_p**2 - self.eps_sigma**2
            ) ** 0.5
            root_positive /= 1 - 4 * self.beta_p**2
            root_negative = (
                1 - 2 * self.beta_p**2
        ...
        if not self.has_complex_root:
            if (
                self.root_positive < self.lower_band_edge
                and self.eps_sigma_wrt_d < 2 * self.beta_p**2 - 1
            ):
            ...
            if (
                self.root_negative > self.upper_band_edge
                and self.eps_sigma_wrt_d > 1 - 2 * self.beta_p**2
            ):
                # Check if the root is below the Fermi level
        ...
        occupied_states = [
            ...
            if self.lower_band_edge < self.eps[i] < upper_bound
        ]
        ...
        if (
            self.has_localised_occupied_state_positive
            and self.has_localised_occupied_state_negative
        ):
